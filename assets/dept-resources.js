/* dept-resources.js
   ...
   Config (set on the page BEFORE this script):
     window.ICF_RESOURCES_SHEET_ID   — Sheet ID (required; empty string = script does nothing)
   ...
   Sheet columns (tab: Department_Content):
   ...
   status values:
     ...
     (anything else or empty) → link button(s)
*/
(function(){
  var SHEET_ID = window.ICF_RESOURCES_SHEET_ID || '';
  var TAB      = (window.ICF_RESOURCES_SHEET_TAB !== undefined)
                   ? window.ICF_RESOURCES_SHEET_TAB
                   : 'Department_Content';
  var COLOR    = window.ICF_DEPT_COLOR || '#475569';
  if(!SHEET_ID) return;

  var slug = window.ICF_RESOURCES_SLUG
    || (window.ICF_DEPT||'').toLowerCase().replace(/\s+/g,'-').replace(/[^a-z0-9-]/g,'')
    || 'dept';
  var wrap = document.getElementById(slug+'-resources-list');
  if(!wrap) return;

  // Light colour variants per accent colour
  var PALETTES = {
  ...
  fetch(url).then(function(r){return r.text();}).then(function(csv){
    ...
    items.forEach(function(row){
      ...
      if(status==='coming soon'||status==='soon'||(!linkUrl&&!contUrl)){
        ...
        // Trigger person modal (church dept pattern)
        ...
        var btns='';
        if(linkUrl){
          var isFile=/\.(pdf|doc|docx|xls|xlsx)(\b|$)/i.test(linkUrl)||linkUrl.includes('/export?format=');
